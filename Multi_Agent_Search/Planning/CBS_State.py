...
class CBS_State:
    ...
    def expand(self):
        # Todo: Your job
        # Get conflict (be sure to take into account dege conflicts when robots switch positions)
        # For each of the two robots in conflict:
        # Create a successor CBS_state with a relevant constraint
        # Deep copy! the plans into the successors
        # Replan the newly constraint robot
        raise NotImplementedError

    # Replan the newly constraint robot
    def replan(self, robot_index):
        #When replanning pass the set of all constraints (from this state up to the root state)
        # Discarded any constraint violating state while performing the single agent planning
        # The single agent planning should now consider duplicate states that share x and y coordinates
        # (as usual) but also the same g value (as explained in class)
        raise NotImplementedError
