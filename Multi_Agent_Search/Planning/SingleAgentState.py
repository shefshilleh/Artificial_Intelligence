...
class SingleAgentState:
    ...
    def __init__(self, p, robot, g, action):
        ...
        self.g = g
        self.action = action
        (leg_1, leg_2) = (abs(self.robot.position_x - self.robot.goal_x), abs(self.robot.position_y - self.robot.goal_y))
        self.h = (leg_1 + leg_2)
        if self.h - leg_1 != 0 and self.h - leg_2 != 0:  # (checking if there is a hypotenuse, then we have at least one corner! which has a cost of 3 to traverse
            self.h += 3
